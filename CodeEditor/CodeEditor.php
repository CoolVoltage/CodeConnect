...
if(isset($Result)) {
	$Owner= $Result[0];
	if(!(isset($_POST['submit']))|| isset($_POST['Version'])) {
	$html = $Result[1];
	$js = $Result[2];
	$css = $Result[3];	
	$ExtRes =$Result[4];
	$Descp = $Result[8];
	$Count_Versions = $Result[9];
}
}
else {
...
<?php if(!isset($_POST['submit'])|| isset($_POST['Version'])) { ?>	
	if (version) {
		html = eval(<?php echo($Result[1]);?>);
		js = eval(<?php echo($Result[2]);?>);
		css = eval(<?php echo($Result[3]);?>);
	var i=1;
	while (!html[version-i]&&i<=version)
	i++;	
	html=html[version-i]?html[version-i]:'<!-- DESCRIPTION/READ ME -->';
	var i=1;
	while (!js[version-i]&&i<=version)
	i++;	
	js=js[version-i]?js[version-i]:'//Use Check to validate your javascript code';
	var i=1;
	while (!css[version-i]&&i<=version)
	i++;	
	css=css[version-i]?css[version-i]:'';
}
<?php } ?>
</script>
<?php	
$iframe = [$Result[1],$Result[3],$Result[2],$_POST['ExternalRes'],$Version,$_POST['submit']];
//print_r($iframe);
